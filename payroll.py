...
from models import PayrollPeriod, Employee, PayrollTransaction
...
def generate_next_pay_run_number(db_session: Session, app_id: int):
    ...
    print(f'Last pay run: doodododo')
    # Query the last payroll period for the company
    last_pay_run = db_session.query(PayrollPeriod).filter_by(app_id=app_id).order_by(
        PayrollPeriod.id.desc()).first()

    print(f'Last pay run: {last_pay_run}')  # Check what the last pay run is
...
def validate_payroll_period(db_session: Session, app_id: int, start_date: date, end_date: date) -> None:
    """
    ...
    print(f'Past the checks now')

    # Check if the period already exists
    # existing_period = db_session.query(PayrollPeriod).filter(
    #     PayrollPeriod.start_date == start_date,
    #     PayrollPeriod.end_date == end_date,
    #     PayrollPeriod.app_id == app_id
    # ).first()
    #
    # if existing_period:
    #     raise ValueError("Payroll period already exists.")

    print(f'Validated dates are {period_frequency}')
...
def calculate_net_salary(gross_salary: float, deductions: float):
    """
